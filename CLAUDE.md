...
(2026-07-27: se eliminaron definitivamente Conteo, Operaciones y Movimientos —
...
- **2026-07-27 — Conteo, Operaciones y Movimientos eliminados definitivamente**: eran código muerto (Conteo llevaba oculto desde 07-18 sin plan de retomarlo; Operaciones/Movimientos placeholders sin implementación). Se borró también `store/conteoStore.ts` (sin otros consumidores) y `crearSolicitud()` de `queries.ts` (solo la usaba Conteo). Si se recupera la necesidad de conteo de inventario, recrear desde el historial de git (`git log --diff-filter=D -- "app/(tabs)/conteo.tsx"`).
- **⚠️ 2026-07-29 — `app/index.tsx` es OBLIGATORIO, no borrarlo jamás**: en la limpieza del 07-27 se eliminó creyendo que `unstable_settings.initialRouteName` lo reemplazaba — FALSO: eso solo ordena los tabs, no da dueño a la ruta `/`. Sin `index.tsx`, el APK de producción renderiza el Sitemap interno de expo-router (fallback de ruta no encontrada) que **crashea en standalone** (`Cannot read property 'origin' of undefined` en SystemInfo, no hay hostUri de Metro) → la app se congela en el splash en TODOS los celulares. En dev NUNCA se nota (el Sitemap no crashea con Metro conectado y el AuthGate redirige antes de verse) — por eso el bug pasó el desarrollo y explotó recién en el APK `12acd244`. Restaurado como `<Redirect href="/escaner" />` (commit `2526a73`).
